 ...
 */
public class SoapLogin extends AsyncTask<String, Void, String> {
    ...
    protected static final String METHOD_NAME = "LOGIN";
    protected static final String SOAP_ACTION = "http://www.wststlutepel.com.br/LOGIN";
    ...
    protected String doInBackground(String... params) {
        ...
        if (params.length > 0)
            try {
                ...
                branch.setName("CFIL");
                branch.setValue(params[1]);
                branch.setType(branch.STRING_CLASS);
                request.addProperty(branch);
                //Usuário
                PropertyInfo pUser = new PropertyInfo();
                pUser.setName("CUSER");
                pUser.setValue(params[2]);
                pUser.setType(pUser.STRING_CLASS);
                request.addProperty(pUser);
                //Senha
                PropertyInfo pPass = new PropertyInfo();
                pPass.setName("CPASS");
                pPass.setValue(params[3]);
                pPass.setType(pPass.STRING_CLASS);
                request.addProperty(pPass);
